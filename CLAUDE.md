...
- `484c471` — feat: câmera do header escaneia sozinha em touch, sem mouse real
- `616931c` — feat: reformula "Como funciona" em "Tutoriais e documentação"
- `04c65f0` — feat: adiciona SectionRail (scroll-spy lateral) e integra no Layout
- `1255e74` — fix: elimina segundo scroll interno trocando overflow-x por clip-path
- `34050ce` — docs: registra pendências do rename e 3 lições técnicas no CLAUDE.md
...
- **Reformulação "Como funciona" → "Tutoriais e documentação"** (`como-funciona.astro`, rota mantida): duas abas (ARIA `tablist`/`tab`/`tabpanel`, navegável por teclado) substituem a página curta de antes — "Tutoriais" (12 tutoriais reais, título + passo a passo + placeholder de GIF, confirmados contra `../camguard-hub/docs/CLAUDE.md` antes de escrever) e "Documentação" (9 entradas de referência, sem placeholder de GIF). Bloco de abertura antigo (eyebrow/h1/diagrama/câmeras compatíveis) removido, texto exato preservado na seção 2 abaixo pra reaproveitar depois.
- **`SectionRail.astro`** (componente novo, compartilhado via `Layout.astro`): navegação lateral tipo scroll-spy — dots verticais fixos no desktop (hover no grupo inteiro expande todos numa lista, não um por vez), botão flutuante + lista no mobile. Critério de seção ativa por posição do topo relativa a uma linha de referência (não por área visível), com caso especial pro fim do documento (ver "Bug real resolvido" abaixo pros dois bugs encontrados e corrigidos nele).
- **Fix do scroll duplicado** (`global.css`): `main` tinha `overflow-x:hidden`, que forçava `overflow-y` computado a virar `auto` e criava uma segunda barra de rolagem interna (ver seção "Bug real resolvido" abaixo pra causa raiz completa). Corrigido trocando por `clip-path:inset(0)` num elemento novo (`.overflow-guard`, dentro de `main`).
- `npm run build` limpo, confirmado repetidas vezes ao longo da série de sessões — a última vez logo após cada um dos 5 commits de hoje (câmera em touch, reformulação Tutoriais/Documentação, SectionRail, fix de scroll/clip-path, e este próprio commit de documentação).

### 2. Ainda não iniciado / não validado

- **Distinção importante, pra não confundir dois tipos diferentes de "validado"**: nesta sessão, o `SectionRail`/abas/scroll-spy passaram por validação real em navegador via Playwright (Chromium automatizado, sem acesso à extensão Chrome do usuário — instalado e removido temporariamente só pra isso) — screenshots revisados, `getComputedStyle`/`getBoundingClientRect` medidos de verdade, não só leitura de código. Isso é diferente de **validação pessoal do usuário no próprio navegador/dispositivo, que ainda não aconteceu pra nada** (nem o que é novo, nem o que já era antigo). Pendente de validação pessoal: o ícone da câmera em 50px (camadas simplificadas por raciocínio de código) — em especial o modo de escaneamento em touch, que nunca rodou nem nos testes automatizados de hoje (o Chromium headless usado sempre bate `hover:hover and pointer:fine`, então só o modo de seguir o cursor foi exercitado, o modo de escanear sozinho continua 100% não testado); o menu hambúrguer mobile com o header agora sticky (z-index/posição do dropdown relativo ao header); e o comportamento de sticky em si. Usuário precisa validar rodando `npm run dev` manualmente antes de considerar essa frente fechada.
...
- Menu hambúrguer no nav abaixo de 768px (medição original: logo + 5 links precisam de ~740px numa linha só pra caber — **desatualizada** desde que o link "Como funciona" virou "Tutoriais e documentação", mais longo; não remedido nesta sessão, sem acesso a navegador; a margem até 768px deve seguir segura, mas vale reconferir na validação visual pendente — mesma ressalva já registrada no comentário de `global.css`).
...
- Setas animadas (ex. CTA com seta pulsando/balançando). Seta estática indicando um fluxo/processo real não é isso — é rótulo de diagrama, não decoração de CTA (sem exemplo vivo no código agora: o `.flow-arrow` do diagrama de `como-funciona.astro` que ilustrava essa distinção foi removido nesta sessão — ver seção 2 de "Sessão em andamento" — texto preservado lá pra reaproveitar depois, mas não está em nenhuma página no momento).
